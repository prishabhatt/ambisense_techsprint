...
// // --- CONFIG ---
// const apiKey = ""; // Leave empty - Gemini calls go through backend
...
//   //     if (result.success) {
//   //       const base64Audio = result.data.audioBase64;
//   //       const wavBlob = pcmToWav(base64Audio, 24000);
//   //       const audio = new Audio(URL.createObjectURL(wavBlob));
//   //       audio.onended = () => setIsSpeaking(false);
//   //       await audio.play();
...
//   //   }
//   // };
//   // Update the readScheduleAloud function in App.jsx:
// const readScheduleAloud = async () => {
//   if (isSpeaking) return;
//   setIsSpeaking(true);
...
//       console.log('✅ Received audio:', result.data.audioBase64.length, 'bytes');
      
//       try {
//         // Convert PCM to WAV
//         const wavBlob = pcmToWav(result.data.audioBase64, 24000);
...
//                   <h3 className="text-2xl flex items-center gap-3 uppercase font-normal"><Settings className="text-indigo-500" /> System Integrity</h3>
//                   <div className="space-y-5">
//                     <ToggleRow label="AI posture tracking engine" active={systemIntegrity.tracking} onClick={!isFamily ? () => setSystemIntegrity({...systemIntegrity, tracking: !systemIntegrity.tracking}) : undefined} disabled={isFamily} darkMode={darkMode} />
//                     <ToggleRow label="Automated alert dispatch" active={systemIntegrity.alerts} onClick={!isFamily ? () => setSystemIntegrity({...systemIntegrity, alerts: !systemIntegrity.alerts}) : undefined} disabled={isFamily} darkMode={darkMode} />
//                   </div>
...
// function NavItem({ icon, label, active, onClick }) {
//   return (
//     <button onClick={onClick} className={`w-full flex items-center gap-5 px-6 py-5 rounded-3xl transition-all duration-300 tracking-wide font-normal ${active ? 'bg-[#4F46E5] text-white shadow-2xl translate-x-1.5' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}>
...
// function MetricCard({ label, value, trend, darkMode }) {
//   return (
//     <div className={`p-8 rounded-[2.5rem] border shadow-sm flex flex-col justify-center transition-all font-normal ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
//       <p className="text-[10px] text-[#64748B] uppercase tracking-wide mb-2 font-normal">{label}</p>
//       <div className="flex items-end justify-between font-normal">
//         <h3 className="text-3xl tracking-tighter">{value}</h3>
//         <div className={`text-[10px] px-3 py-1 rounded-full bg-[#ECFDF5] text-[#059669] border border-emerald-100 uppercase`}>
//           {trend}
//         </div>
//       </div>
//     </div>
...
//         // Method 1: Direct base64 playback (most reliable)
...
//           // Fallback to Method 2: Blob playback
...
//     }
//   };
// // Replace the testAudioPlayback function in your component:
// const testAudioPlayback = () => {
//   console.log('🔊 Testing audio playback...');
  
//   // Method 1: Test with a simple beep sound using Web Audio API
//   try {
...
//     console.log('✅ Web Audio API test sound playing...');
...
//       const beepData = 'data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEAQB8AAEAfAAABAAgAZGF0YQAAAAA=';
//       audio.src = beepData;
      
//       audio.play()
//         .then(() => console.log('✅ HTML5 Audio test passed'))
//         .catch(e => console.error('❌ HTML5 Audio failed:', e));
//     }, 1100);
    
//   } catch (error) {
//     console.error('❌ Web Audio API failed:', error);
    
//     // Fallback: Simple beep with oscillator
//     const beep = () => {
...
//                   </div>
//                   <img 
//                     src="https://images.unsplash.com/photo-1513151233558-d860c5398176?auto=format&fit=crop&w=1200" 
//                     className={`w-full h-full object-cover transition-all duration-1000 ${isPrivacyMasked ? 'blur-3xl' : 'opacity-30 grayscale'}`} 
//                     alt="View" 
//                   />
//                   {!isPrivacyMasked && (
...
// <<<<<<< HEAD
//                       <button onClick={readScheduleAloud} disabled={isSpeaking} className="p-2 bg-indigo-50 text-indigo-600 rounded-xl">
//                         {isSpeaking ? <Loader2 size={16} className="animate-spin" /> : <Volume2 size={16} />}
//                       </button>
//                     </div>
//                     <div className="space-y-2">
//                       {medications.map((m, i) => (
//                         <div key={i} className={`p-3 rounded-2xl border flex items-center gap-4 ${m.completed ? 'bg-emerald-50 border-emerald-100 opacity-60' : darkMode ? 'bg-slate-700/50 border-slate-600' : 'bg-slate-50 border-slate-100'}`}>
//                           <div className={`w-8 h-8 rounded-full flex items-center justify-center ${m.completed ? 'bg-emerald-500 text-white' : 'bg-indigo-100 text-indigo-600'}`}>
//                             {m.completed ? <CheckCircle size={14} /> : <Pill size={14} />}
//                           </div>
//                           <div>
//                             <p className="text-xs font-normal">{m.med}</p>
//                             <span className="text-[9px] text-slate-400 uppercase font-normal">{m.time} • {m.dose}</span>
// =======
//                       <button 
...
//                               {m.time} • {m.dose}
//                             </span>
// >>>>>>> 4b9fd49944a92c416d9d3b3409708fdf8e2d054e
//                           </div>
//                         </div>
//                       ))}
//                     </div>
// <<<<<<< HEAD
//                     <button onClick={readScheduleAloud} disabled={isSpeaking} className="w-full mt-4 py-2 text-[10px] uppercase text-indigo-500 hover:underline transition-all">
//                       Listen to schedule
//                     </button>
//                     {/* Debug button */}
//                     <button onClick={testAudioPlayback} className="w-full mt-2 py-2 text-[10px] uppercase text-blue-500 hover:underline transition-all">
//                       🔧 Test Audio
//                     </button>
// =======
...
//                 <MetricCard label="Stability" value="88%" trend="Normal" darkMode={darkMode} />
//                 <MetricCard label="Active effort" value={wellnessRange === 'Day' ? "42m" : "280m"} trend="+12%" darkMode={darkMode} />
...
//                         <Area type="monotone" dataKey="intensity" stroke="#4F46E5" strokeWidth={5} fillOpacity={1} fill="url(#colorIntensity)" />
//                       </AreaChart> : 
...
//                         <YAxis axisLine={false} tickLine={false} tick={{fill: '#64748B', fontSize: 11}} />
//                         <Bar dataKey="steps" fill="#4F46E5" radius={[15, 15, 0, 0]} />
//                       </BarChart>
//                     }
//                   </ResponsiveContainer>
//                 </div>
